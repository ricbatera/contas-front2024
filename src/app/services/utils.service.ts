...
import { Classificacao } from 'src/model/general/classificacao';
import { meses } from 'src/model/general/meses';
import { setMesAnoInicialFinal } from '../sistema/store/sistema.actions';
...
export class UtilsService {
  ...
  public loadDateYear(status: boolean) {
    if (status) {
      const hoje = new Date();
      const payload: MesAno = {
        mesStart: hoje.getMonth() + 1,
        anoStart: hoje.getFullYear(),
        mesEnd: hoje.getMonth() + 1,
        anoEnd: hoje.getFullYear() + 1
      }
      this.store.dispatch(setMesAnoInicialFinal({ payload }))
    }
  }
